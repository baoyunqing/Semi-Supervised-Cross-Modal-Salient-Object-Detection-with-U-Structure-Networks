...
class BASNet(nn.Module):
    ...
    def forward(self,x, captions):
        ...
        hx = self.resb6_2(hx)
        h6 = self.resb6_3(hx)
        #h6 = h6 + self.cmsa(h6,embeddings,ch = h6.shape[1],vf_h = h6.shape[2], vf_w = h6.shape[3])

        ## -------------Bridge-------------
        hx = self.relubg_1(self.bnbg_1(self.convbg_1(h6))) # 8
        #hx = hx + self.cmsa(hx,embeddings,ch = hx.shape[1],vf_h = hx.shape[2], vf_w = hx.shape[3])
        hx = self.relubg_m(self.bnbg_m(self.convbg_m(hx)))
        hx = hx + self.cmsa(hx,embeddings,ch = hx.shape[1],vf_h = hx.shape[2], vf_w = hx.shape[3])
        hbg = self.relubg_2(self.bnbg_2(self.convbg_2(hx)))
        #print('hbg: ',hbg.shape)
        #hbg = hbg + self.cmsa(hbg,embeddings,ch = hbg.shape[1],vf_h = hbg.shape[2], vf_w = hbg.shape[3])
        ...
        ## -------------Refine Module-------------
        dout = self.refunet(d1,embeddings) # 256
